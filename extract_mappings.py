...
def read_bernard_scraped():
    ...
    with open('bernard/source/scraped.json') as f:
        for line in f:
...
for bibl_id in dists:
    for doc_id, nt_idx in dists[bibl_id]:
        if dists[bibl_id][doc_id, nt_idx] / len(by_doc_id[doc_id][nt_idx].split()) > 0.9:
...
with open('bernard/output/mappings.csv', 'w') as f:
